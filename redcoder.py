#!/usr/bin/env python3
"""
Redcoder — offline agentic coding CLI.
...
directories, search, and run shell commands on this PC — all offline.
...
Run it by typing `redcoder` in any terminal (see redcoder.cmd), or:
    python redcoder.py [flags] [prompt...]
...
Flags (Claude-Code style):
    ...
    -m, --model NAME                use a different Ollama model (default: redcoder-max)
...
Voice: if Whisper + ffmpeg + a mic are present, hold Space at an empty prompt to talk;
release to transcribe (offline, on-GPU) and submit. The recording is deleted after.
...
DEFAULT_MODEL = "redcoder-max"   # best quality that fits this box; /model to switch
NUM_CTX = 32768         # stable window (64K crashed the runner on this box); auto-compaction handles longer
...
the user's own Windows PC. You complete software and security-lab tasks by using tools \
...
- run_shell        {"command": str}                               run a PowerShell command
    ...
    def _stream_plan(self):
        while self.plan_i < len(self.plan):
            ...
            if self.val_start is None:
                vm = re.search(r'"' + key + r'"\s*:\s*"', self.raw)
                if not vm:
                    return  # this value hasn't started streaming yet
                self.val_start = vm.end()
            frag = self.raw[self.val_start:]
            end = _json_str_end(frag)
            escaped = frag if end is None else frag[:end]
            if end is None:
                trail = len(escaped) - len(escaped.rstrip("\\"))
                if trail % 2 == 1:
                    escaped = escaped[:-1]
            try:
                ...
                return
            new = decoded[self.val_shown:]
            self.val_shown = len(decoded)
            self.line_buf += new
            while "\n" in self.line_buf:
            ...
            # advance to the next value in the plan
            self.plan_i += 1
            self.val_start = None
            self.val_shown = 0
...
def preflight(model):
    try:
        ...
        return False, (f"Ollama is up but model '{model}' is not installed. "
                       f"Create it: ollama create {model} -f config/Modelfile.redcoder")
    except Exception:
